...
### Part 2 — 확대 표본 재검증 (2026-07-08 실행)

**절차**: `main.py backtest --strategy <id>`(Phase-4 전체 게이트, US 154종목
확대 표본 GATING / KR 20종목 보고용)를 zscore_meanrev·wyckoff_spring에 각각
실행. pullback은 사전등록 그리드(`tests/compare_pullback_veto.py`,
dist_veto_bars ∈ {5, 10} — 결과를 보기 전에 고정)로 분산 거부 필터를 비교
(정보용 — 활성화에는 Phase-4 전체 게이트 별도 통과 필요). 모든 결과는 과거
성과이며 미래 수익을 보장하지 않고, 생존 편향(현재 유니버스 기준, 상장폐지
종목 제외로 낙관적 왜곡)이 적용된다.

**요약표**:

| 전략 | OoS n | OoS PF | OoS WR | 게이트 | 결정 |
|---|---|---|---|---|---|
| zscore_meanrev | 25 | 1.11 | 52.0% | 6/6 PASS | **활성화** (n≥20 충족) |
| wyckoff_spring | 24 | 0.59 | 33.3% | 2/6 FAIL | 비활성 유지 (내용 탈락) |
| pullback (veto 그리드) | 39–40 | 0.62–0.66 | 37.5–38.5% | 그리드만 (정보용) | 비활성 유지, veto 미채택 |

**2a. zscore_meanrev — 전 게이트 PASS → 활성화.** 7/2 탈락 사유였던 표본
부족(n=14)이 확대 표본으로 해소(n=25 ≥ 20), 나머지 게이트도 전부 유지.
verbatim 원문 출력:

```
2026-07-08 17:43:07,614 INFO src.analysis.registry registry: 6 strategies loaded (enabled_only=False)
2026-07-08 17:43:10,728 INFO src.backtest.run_validation frames[us]: 154/160 sample tickers usable
2026-07-08 17:43:11,820 INFO src.backtest.run_validation frames[kr]: 20/20 sample tickers usable
2026-07-08 17:43:13,661 INFO src.backtest.validation [zscore_meanrev] generating entry plans for 154 tickers...
2026-07-08 17:43:24,804 INFO src.backtest.validation [zscore_meanrev] gates: {'G1_oos_pf_gt_1': True, 'G2_wr_holdup': True, 'G3_walk_forward': True, 'G4_mc_mdd_bound': True, 'G5_sensitivity': True, 'G6_min_oos_trades': True} -> PASS
2026-07-08 17:43:24,805 INFO src.backtest.validation [zscore_meanrev] generating entry plans for 20 tickers...
2026-07-08 17:43:28,610 INFO src.backtest.validation [zscore_meanrev] gates: {'G1_oos_pf_gt_1': False, 'G2_wr_holdup': False, 'G3_walk_forward': False, 'G4_mc_mdd_bound': True, 'G5_sensitivity': False, 'G6_min_oos_trades': False} -> FAIL

==============================================================================
VALIDATION RUN — US sample 154 (GATING) / KR sample 20 (보고용)
주의: 백테스트는 현재 유니버스 기준 — 상장폐지 종목 제외로 생존 편향(낙관적 왜곡) 존재
==============================================================================

### zscore_meanrev — PASS → enabled
| metric | IS | OoS |
|---|---|---|
| trades | 89 | 25 |
| win rate | 61.8% | 52.0% |
| profit factor | 1.77 | 1.11 |
- walk-forward PF: W1=1.63(n=24), W2=1.83(n=62), W3=1.37(n=30)
- Monte Carlo(1000): 5%ile final equity x1.03, worst-tail MDD 7.6%
- sensitivity: z_entryx0.8→PF 1.358✓, z_entryx1.2→PF 4.963✓, vol_multx0.8→PF 1.93✓, vol_multx1.2→PF 0.914✓
- regimes: bull: n=56, PF=0.88, bear: n=35, PF=3.38, sideways: n=23, PF=2.06
- benchmark (index B&H same span): +72.5%
- gates: ✅G1_oos_pf_gt_1 ✅G2_wr_holdup ✅G3_walk_forward ✅G4_mc_mdd_bound ✅G5_sensitivity ✅G6_min_oos_trades
  [KR 참고] 표본 내 거래 없음
```

주: 로그의 KR 20종목 게이트 라인(FAIL)은 보고용 러너의 부산물 — KR 표본 내
거래 0건이라 통계적 의미 없음("표본 내 거래 없음"). GATING은 US 결과.

정직 고지: OoS PF 1.11은 7/2의 1.77(n=14)보다 크게 낮다 — 표본이 늘며
소표본 낙관치가 희석된 것으로, 통과선(>1)을 넘되 여유는 얇다. bull 구간
PF 0.88(<1)로 수익원이 bear/sideways에 편중된 평균회귀 특성도 그대로다.
과거 성과이며 미래 수익 보장 없음.

**2b. pullback 분산 거부 필터 그리드 — 미채택, 비활성 유지.** 사전등록 그리드
결과, 어떤 변형도 OoS PF를 1 근처로 끌어올리지 못함(최고 0.66). 결정 규칙
("명확히 PF≥1 영역으로 이동 시에만 전체 게이트 실행")에 따라 Phase-4 전체
게이트는 실행하지 않음. verbatim 출력 (아래 stat 라인은 원문 그대로이며,
backtesting.py의 tqdm 진행바 캐리지리턴 잔상만 제거함 —
`tr '\r' '\n' < /tmp/val_pullback_veto.txt | grep -v "Backtest.run" | grep -v -E '^\s*$'`):

```
pullback veto grid — US sample 154 · 생존 편향 주의, 과거 성과
variant                  OoS n   OoS WR   OoS PF   IS PF
baseline                    40    37.5%     0.62    0.93
dist_veto_bars=5            40    37.5%     0.62    0.93
dist_veto_bars=10           39    38.5%     0.66    0.95
```

해석: veto=5는 베이스라인과 완전 동일(OoS 거부 발동 무영향), veto=10은
OoS 거래 1건 제거로 PF +0.04·WR +1.0%p — 방향은 맞으나 미미하며 PF 0.66은
통과선(>1)에서 멀다. 설계 시점의 솔직한 기대치("베이스라인 PF는 통과선에서
멀어 탈락 가능성 높음") 그대로의 결과. dist_veto_bars는 YAML에 미설정
(파라미터 부재 = 기존 동작 파리티 유지).

**2c. wyckoff_spring — FAIL → 비활성 유지.** 표본은 확보됐으나(OoS n=24 ≥ 20,
G6 통과) 내용으로 탈락: OoS PF 0.59, WR 33.3%, walk-forward 1/3, 민감도 3/4.
설계 시점의 솔직한 기대치("내용 탈락 가능") 그대로. verbatim 원문 출력:

```
2026-07-08 17:43:40,395 INFO src.analysis.registry registry: 6 strategies loaded (enabled_only=False)
2026-07-08 17:43:43,426 INFO src.backtest.run_validation frames[us]: 154/160 sample tickers usable
2026-07-08 17:43:44,483 INFO src.backtest.run_validation frames[kr]: 20/20 sample tickers usable
2026-07-08 17:43:46,126 INFO src.backtest.validation [wyckoff_spring] generating entry plans for 154 tickers...
2026-07-08 17:55:52,674 INFO src.backtest.validation [wyckoff_spring] gates: {'G1_oos_pf_gt_1': False, 'G2_wr_holdup': False, 'G3_walk_forward': False, 'G4_mc_mdd_bound': True, 'G5_sensitivity': False, 'G6_min_oos_trades': True} -> FAIL
2026-07-08 17:55:52,674 INFO src.backtest.validation [wyckoff_spring] generating entry plans for 20 tickers...
2026-07-08 18:00:25,198 INFO src.backtest.validation [wyckoff_spring] gates: {'G1_oos_pf_gt_1': True, 'G2_wr_holdup': True, 'G3_walk_forward': True, 'G4_mc_mdd_bound': True, 'G5_sensitivity': False, 'G6_min_oos_trades': False} -> FAIL
...
### wyckoff_spring — FAIL → disabled
| metric | IS | OoS |
|---|---|---|
| trades | 42 | 24 |
| win rate | 59.5% | 33.3% |
| profit factor | 1.40 | 0.59 |
- walk-forward PF: W1=0.33(n=9), W2=2.48(n=31), W3=0.53(n=26)
- Monte Carlo(1000): 5%ile final equity x0.89, worst-tail MDD 13.8%
- sensitivity: vpa.vol_multx0.8→PF 1.403✓, vpa.vol_multx1.2→PF 1.107✓, vpa.exhaust_ratiox0.8→PF 0.898✗, vpa.exhaust_ratiox1.2→PF 1.627✓
- regimes: bull: n=40, PF=0.91, bear: n=8, PF=2.49, sideways: n=18, PF=0.85
- benchmark (index B&H same span): +72.5%
- gates: ❌G1_oos_pf_gt_1 ❌G2_wr_holdup ❌G3_walk_forward ✅G4_mc_mdd_bound ❌G5_sensitivity ✅G6_min_oos_trades
  [KR 참고] IS n=5 PF=4.72 / OoS n=1 PF=inf
```

"매집 완료 진입" 요청(소유자 요청 3)의 정공법이었으나 현재 파라미터/데이터로는
근거 부족 — 정직하게 비활성 유지. IS 1.40 → OoS 0.59의 큰 열화는 과적합
신호이기도 하다.

**활성 전략 수 확인**: breakout + zscore_meanrev = **2개 활성** (7-cap 이내,
활성화 규칙 준수 — zscore_meanrev는 전 게이트 통과 후에만 활성화됨).
