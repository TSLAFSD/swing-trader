...
class SqueezeStrategy(BaseStrategy):
    ...
    def evaluate(self, df: pd.DataFrame, ticker: str, name: str, market: str) -> Signal | None:
        ...
        required = ["squeeze_on", "vol_ma20", "linreg_slope20", "atr14"]
        if pd.isna(row[["vol_ma20", "linreg_slope20", "atr14"]]).any():
            return None
        # Squeeze was ON for >= N consecutive days ending yesterday, releases today.
        recent = df["squeeze_on"].iloc[-1 - n : -1]
        squeeze_held = len(recent) == n and bool(recent.all())
        released = not bool(row["squeeze_on"])
        conditions = (
            squeeze_held
            and released
            and row["close"] > prev["high"]
            and row["volume"] > p["vol_mult"] * row["vol_ma20"]
            and row["linreg_slope20"] > 0
        )
        if not conditions:
            return None
